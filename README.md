...
1. **Launch from the catapult** — your plane sits on a slingshot launcher. **Drag back to pull the sling** (the band stretches and the plane tilts to aim): the **direction** you drag sets the **launch angle** (steep, straight-down pull = high arc; shallow, sideways pull = flat & fast), and the **distance** sets power. A dotted **trajectory arc** previews the shot; release in the sweet-spot band for a **PERFECT LAUNCH** (extra speed + fuel). Releasing fires the catapult with a smoke-and-spark kick.
